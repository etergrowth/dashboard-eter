...
import type { LeadSandboxInsert, LeadSource, LeadStatus } from '../../../types/sandbox';

interface LeadFormProps {
  onSubmit: (lead: LeadSandboxInsert & { status?: LeadStatus }) => void;
  onCancel: () => void;
  initialData?: Partial<LeadSandboxInsert & { status?: LeadStatus }>;
  isLoading?: boolean;
}
...
const statuses: { value: LeadStatus; label: string }[] = [
  ...
  { value: 'engaged', label: 'Contactado' },
  { value: 'qualified', label: 'Qualificado' },
  { value: 'crm_ready', label: 'Pronto CRM' },
  { value: 'dead', label: 'Perdido' },
];

export function LeadForm({ onSubmit, onCancel, initialData, isLoading }: LeadFormProps) {
  const [formData, setFormData] = useState<LeadSandboxInsert & { status?: LeadStatus }>({
  ...
  // Atualizar formData quando initialData mudar (modo edição)
  useEffect(() => {
    if (initialData) {
      setFormData({
        name: initialData.name || '',
  ...
  const handleChange = (field: keyof (LeadSandboxInsert & { status?: LeadStatus }), value: any) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-2 pt-4">
        ...
        <Button
          ...
          className="flex-1"
        >
          {isLoading ? 'A guardar...' : initialData ? 'Atualizar Lead' : 'Guardar Lead'}
        </Button>
      </div>
    </form>
  );
